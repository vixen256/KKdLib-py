...
inline void
Decode565 (HDRColorA *pColor, const u16 w565) noexcept {
...
inline u16
Encode565 (const HDRColorA *pColor) noexcept {
	HDRColorA Color;
	...
	w = static_cast<u16> ((static_cast<i32> (Color.r * 31.0f + 0.5f) << 11) | (static_cast<i32> (Color.g * 63.0f + 0.5f) << 5) |
	                      (static_cast<i32> (Color.b * 31.0f + 0.5f) << 0));

	return w;
}

void
OptimizeRGB (HDRColorA *pX, HDRColorA *pY, const HDRColorA *pPoints, u32 cSteps, u32 flags) noexcept {
...
void
EncodeBC1 (D3DX_BC1 *pBC, const HDRColorA *pColor, bool bColorKey, f32 threshold, u32 flags) noexcept {
	assert (pBC && pColor);
	...
	for (i = 0; i < NUM_PIXELS_PER_BLOCK; ++i) {
		if ((3 == uSteps) && (pColor[i].a < threshold)) {
			...
			else iStep = uint32_t (pSteps[uint32_t (fDot + 0.5f)]);
...
void
D3DXEncodeBC3 (u8 *pBC, const HDRColorA *pColor, u32 flags) noexcept {
	assert (pBC && pColor);
	...
	float fMinAlpha = pColor[0].a;
	float fMaxAlpha = pColor[0].a;

	for (size_t i = 0; i < NUM_PIXELS_PER_BLOCK; ++i) {
		float fAlph = pColor[i].a;
		if (flags & BC_FLAGS_DITHER_A) fAlph += fError[i];
		...
		if (flags & BC_FLAGS_DITHER_A) {
			...
			if (i < 12) {
				if (i & 3) fError[i + 3] += fDiff * (3.0f / 16.0f);
...
#ifdef COLOR_WEIGHTS
	if (0.0f == fMaxAlpha) {
		EncodeSolidBC1 (&pBC3->dxt1, pColor);
		pBC3->alpha[0] = 0x00;
		pBC3->alpha[1] = 0x00;
	...
	// RGB part
	EncodeBC1 (&pBC3->bc1, pColor, false, 0.f, flags);
	...
	if (flags & BC_FLAGS_DITHER_A) memset (fError, 0x00, NUM_PIXELS_PER_BLOCK * sizeof (float));
	...
	for (size_t iSet = 0; iSet < 2; iSet++) {
		...
		for (size_t i = iMin; i < iLim; ++i) {
			...
			uint32_t iStep;
			if (fDot <= 0.0f) iStep = ((6 == uSteps) && (fAlph <= fStep[0] * 0.5f)) ? 6u : 0u;
			else if (fDot >= fSteps) iStep = ((6 == uSteps) && (fAlph >= (fStep[1] + 1.0f) * 0.5f)) ? 7u : 1u;
			else iStep = uint32_t (pSteps[uint32_t (fDot + 0.5f)]);
			...
			if (flags & BC_FLAGS_DITHER_A) {
				...
				if (3 != (i & 3)) fError[i + 1] += fDiff * (7.0f / 16.0f);
				...
				if (i < 12) {
					...
					if (3 != (i & 3)) fError[i + 5] += fDiff * (1.0f / 16.0f);
				}
			}
		}
